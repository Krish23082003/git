...
async function generateNewBlogTopicUsingAi() {
	try {
		...
		const response = await axios.post(
			'https://vivazureaihub1913474007.openai.azure.com/openai/deployments/gpt-4o/chat/completions?api-version=2023-03-15-preview', //Azure OpenAI endpoint
			{
				messages: [
					{
						role: 'system',
		...
		return {
			success: true,
			blogtopic: newFetchedTopics[0]?.Topic || null,
			blogdescr: newFetchedTopics[0]?.Description || null
		};
	} catch (error) {
...
async function generateImage() {
	try {
		const response = await axios.post(
			'https://vivazureaihub1913474007.openai.azure.com/openai/deployments/dall-e-3/images/generations?api-version=2024-02-01',
			{
				prompt: imgprompt,
				n: 1,
				size: '1024x1024'
...
// end Function to generate an image
...
} // end Function to download the image
